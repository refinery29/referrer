...
describe('when user comes by a link from other site', function() {
	...
	describe('and that site is a known site', function() {
		it ('should log R29.Referrers omitting domain zone and log known params', function() {
			...
			expect(referrers.contains('google?q=hey bro')).toBe(true)
			expect(referrers.contains('google?q=hey%20bro')).toBe(true)
		})
	});
});
...
describe('when user comes from email', function() {
	describe('and referrer parameter is set', function() {
		it ('should log the fact', function() {
			...
			expect(referrers.contains('email')).toBe(true);
		})
	})
	...
	describe('and utm_campaign param is given', function() {
		it ('should log campaign', function() {
			...
			expect(referrers.contains('email')).toBe(false);
		})
	})
	...
	describe('multiple sources', function() {
		it ('should concatenate string', function() {
			...
			expect(referrers.contains('telepathy')).toBe(true);
			expect(referrers.contains('google')).toBe(true);
			expect(referrers.contains('google?q=a%26b%3Fc%3Dd')).toBe(true);
			expect(referrers.contains('google?q=a&b?c=d')).toBe(false);
			expect(referrers.contains('google?q=носки')).toBe(true);
			expect(referrers.contains('google?q=н*и')).toBe(true);
			expect(referrers.contains('google?q=н*ц')).toBe(false);
			expect(referrers.contains('google?q=носки драные')).toBe(false);
			expect(referrers.contains('google?q=%D0%BD%D0%BE%D1%81%D0%BA%D0%B8')).toBe(true);
			expect(referrers.contains('google?q=носки')).toBe(true);
			expect(referrers.contains('google?q=носки драные')).toBe(false);
			expect(referrers.contains('facebook')).toBe(true);
			expect(referrers.contains('facebook/')).toBe(false);
			expect(referrers.contains('facebook/something')).toBe(true);
			expect(referrers.contains('facebook/someone')).toBe(false);
			expect(referrers.contains('facebook/something/123123123123')).toBe(false);
			expect(referrers.contains('facebook/someone/123123123123')).toBe(false);
			expect(referrers.contains('sales.co.uk')).toBe(true);
			expect(referrers.contains('sales.co.uk/')).toBe(true);
			expect(referrers.contains('sales.com')).toBe(false);
			expect(referrers.contains('sales.com/')).toBe(false);
			expect(referrers.contains('google')).toBe(true);
			expect(referrers.contains('google?q=something')).toBe(true);
			expect(referrers.contains('google?q=*thing')).toBe(true);
			expect(referrers.contains('google?q=*thong')).toBe(false);
			expect(referrers.contains('google?q=*th[oi]ng')).toBe(true);
			expect(referrers.contains('google?q=other')).toBe(false);
			expect(referrers.contains('bing')).toBe(false)
			expect(referrers.contains('pinterest/somebody')).toBe(true)
			expect(referrers.contains('pinterest/someone')).toBe(false)
			expect(referrers.contains('pinterest')).toBe(true)
			expect(referrers.contains('pinterest/')).toBe(true)
			expect(referrers.contains('pinterest/pinner')).toBe(true)
			expect(referrers.contains('pinterest/pinner/123')).toBe(false)
			expect(referrers.contains('pinterest/pin')).toBe(false)
			expect(referrers.contains('pinterest/pin/123')).toBe(false)
		})
	})
})
})